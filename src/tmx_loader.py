...
#-------------------------------------------------------------------------------
def set_slices(world_map, SLICE_SIZE, SLICE_SIZE_PIXEL):
    slices = []    
    for n_slices in xrange(0, world_map.width, SLICE_SIZE):
        surf = pygame.Surface((SLICE_SIZE_PIXEL, world_map.pixel_height))
        for layer in world_map.layers[:]:
            if layer.visible:
                idx = 0
                # loop over all tiles
                for ypos in xrange(0, layer.height):
                    for xpos in xrange(0, SLICE_SIZE):
                        # add offset in number of tiles
                        x = (xpos + layer.x) * world_map.tilewidth
                        #if n_slices == 0:
                        #    x += world_map.tilewidth
                        y = (ypos + layer.y) * world_map.tileheight
                        # get the gid at this position
                        img_idx = layer.content2D[xpos + n_slices][ypos]
                        idx += 1
                        if img_idx:
