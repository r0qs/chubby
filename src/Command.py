...
class CommandHandler(object):
    ...
    def refresh_state(self, in_key):
        ...
        if self.actual_state == 5: self.doRoll          = True; self.final_state = self.actual_state
        elif self.actual_state == 8: self.doSprint      = True; self.final_state = self.actual_state
        elif self.actual_state == 10: self.doGetDown    = True; self.final_state = self.actual_state
        elif self.actual_state == 11: self.doJump       = True; self.final_state = self.actual_state
        elif self.actual_state == 13: self.doClimb      = True; self.final_state = self.actual_state
